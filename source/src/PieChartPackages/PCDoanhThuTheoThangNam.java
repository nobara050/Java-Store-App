...
public class PCDoanhThuTheoThangNam extends JFrame {
        ...
        comboBoxYear.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (comboBoxYear.getSelectedIndex() >= 0) {
                    ...
                    try {
                        comboBoxMonth.removeAllItems(); // Clear previous month items
    ...
    //Lấy dữ liệu cho biểu đồ theo năm và tháng đã chọn
    private void updatePieChart(int year, int month) throws SQLException {
        pieDataset.clear();
        PreparedStatement p = MyConnect.getConnection().prepareStatement(
                "SELECT sp.tensp, SUM(cthd.ThanhTien) AS tongtienSUM FROM cthoadon cthd " +
                "JOIN sanpham sp ON cthd.MaSP = sp.MaSP JOIN HOADON HD on HD.MAHD=cthd.MAHD " +
                "WHERE EXTRACT(YEAR FROM HD.NgayLap) = ? AND EXTRACT(MONTH FROM HD.NgayLap) = ? " +
                "GROUP BY sp.tensp ORDER BY sp.tensp DESC");
        p.setInt(1, year);
        p.setInt(2, month);
        ResultSet r = p.executeQuery();
        while (r.next()) {
            String productName = r.getString("tensp");
            double totalAmount = r.getDouble("tongtienSUM");
    ...
    private void showPieChart() {
        ...
        pieDataset.setValue("No Data", 100); // Initial placeholder data

        pieChart = ChartFactory.createPieChart3D("Thống kê doanh số từng tháng theo năm", pieDataset, true, true, false);
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(() -> {
            try {
                PCDoanhThuTheoThangNam frame = new PCDoanhThuTheoThangNam();
